...
export async function activate(context: vscode.ExtensionContext) {
	await tryActivate(context).catch(err => {
		setStatus(Status.Error, `Failed to activate Sprocket extension: ${err.message}`);
		throw err;
	});
}

function registerCommands(context: vscode.ExtensionContext) {
	context.subscriptions.push(vscode.commands.registerCommand("sprocket.showChannel", (_) => {
		channel.show();
	}));

	context.subscriptions.push(vscode.commands.registerCommand("sprocket.restartServer", restartServer));
}
...
async function startServer() {
	...
	if (path) {
		channel.appendLine(`Spawning \`${path}\` with arguments \`${JSON.stringify(args)}\``);
	} else {
		channel.appendLine(`Spawning \`sprocket\` with arguments \`${JSON.stringify(args)}\``);
	}

	let serverOptions: ServerOptions = { command: path, args: args, transport: TransportKind.stdio };
	client = new LanguageClient('sprocket', 'Sprocket', serverOptions, clientOptions);
...
async function onDidChangeConfiguration(event: vscode.ConfigurationChangeEvent) {
	if (event.affectsConfiguration('sprocket.server')) {
		const message = "Changing server options requires a Sprocket server restart";
